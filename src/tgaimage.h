...
struct TGAColor {
	...
	// grey scale color
	TGAColor(const std::uint8_t Grey) : bgra{Grey,0,0,0} {}
    std::uint8_t& operator[](const int i) { return bgra[i]; }
    TGAColor& operator * (const float coef) { for (int i=3; i--;bgra[i]*=coef); return *this;}
};
...
struct TGAImage {
    ...
    TGAImage() = default;
    TGAImage(const int w, const int h, const int bpp, const int c);
